...
export const Content: React.FC = () => {
  ...
  return sessionData?.user ? <ContentWithAuth /> : <ContentWithoutAuth />;
};

export const ContentWithoutAuth = () => {
  return (
    <div className="flex h-[80vh] flex-col items-center justify-center">
      <h1 className="text-center text-7xl font-semibold">
        A habit tracker for daily goals
      </h1>
      <h2 className="mt-6 mb-14 text-center text-4xl font-medium text-gray-500">
        Develop good habits by visualizing daily progress
      </h2>
    </div>
  );
};

export const ContentWithAuth = () => {
  const { data: sessionData } = useSession();
